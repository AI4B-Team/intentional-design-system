...
-- Add missing columns to unified_actions that the trigger and app code expect
ALTER TABLE public.unified_actions 
  ADD COLUMN IF NOT EXISTS owner_mode TEXT NOT NULL DEFAULT 'human' CHECK (owner_mode IN ('human', 'hybrid', 'ai_agent')),
  ADD COLUMN IF NOT EXISTS channel TEXT CHECK (channel IN ('call', 'sms', 'email', 'in_app', 'portal')),
